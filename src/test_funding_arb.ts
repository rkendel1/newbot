...
class MockFundingRateMonitor {
  ...
  async detectOpportunity(mockData: MockFundingRates, threshold: number) {
    const { apexRate, binRate } = await this.getFundingRates(mockData);
...
class TestPositionManager {
  ...
  openPosition(opportunity: any): Position {
    ...
    const position: Position = {
      ...
      entryTime: new Date(),
      apexEntryPrice: 45000, // Mock price
      coinbaseEntryPrice: 45010, // Mock price
      apexFundingRate: 0.0003,
...
async function runTests() {
  ...
  const mockData1: MockFundingRates = {
    apexRate: 0.0005,  // 0.05% per 8h
    binRate: 0.0002, // 0.02% per 8h
    timestamp: new Date()
  };
  
  const spread1 = mockData1.apexRate - mockData1.binRate;
  console.log(`Bybit Rate: ${(mockData1.apexRate * 100).toFixed(4)}%`);
  ...
  if (opportunity1) {
    ...
    console.log(`   Strategy: ${opportunity1.sideBybit} on Apex, ${opportunity1.sideBin} on Coinbase`);
  } else {
  ...
  const mockData2: MockFundingRates = {
    apexRate: 0.00015,  // 0.015% per 8h
    binRate: 0.00012, // 0.012% per 8h
    timestamp: new Date()
  };
  
  const spread2 = mockData2.apexRate - mockData2.binRate;
  console.log(`Bybit Rate: ${(mockData2.apexRate * 100).toFixed(4)}%`);
  ...
  const spread3 = mockData3.apexRate - mockData3.binRate;
  ...
  const mockData6: MockFundingRates = {
    apexRate: 0.0001,   // 0.01% per 8h (lower)
    binRate: 0.0004,  // 0.04% per 8h (higher)
    timestamp: new Date()
  };
  
  const spread6 = mockData6.apexRate - mockData6.binRate;
  console.log(`Bybit Rate: ${(mockData6.apexRate * 100).toFixed(4)}%`);
  ...
  if (opportunity6 && opportunity6.sideBybit === 'SHORT' && opportunity6.sideBin === 'LONG') {
    ...
    console.log(`   Strategy: ${opportunity6.sideBybit} on Apex, ${opportunity6.sideBin} on Coinbase`);
  ...
  for (let i = 0; i < 100; i++) {
    const mockDataStress: MockFundingRates = {
      apexRate: 0.0002 + Math.random() * 0.0004,
